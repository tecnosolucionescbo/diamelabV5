...
RETURNS TRIGGER AS $$
BEGIN
    INSERT INTO public.profiles (id, full_name, role, sede)
    VALUES (
        NEW.id,
...
-- IMPORTANTE: Estos usuarios deben crearse primero en 
-- Authentication > Users de Supabase, luego ejecutar estos inserts
-- ============================================================
...
-- cuando crees los usuarios manualmente en el panel.
...
-- Si prefieres crear por SQL (método recomendado para datos iniciales):
-- Descomenta y ejecuta lo siguiente DESPUÉS de habilitar email confirm:
...
VALUES (
    ...
    NOW(),
    '{"full_name": "Administrador Diamelab", "role": "admin", "sede": "Todas"}'::jsonb
);

-- VENDEDOR CIUDAD BOLIVAR
INSERT INTO auth.users (id, email, encrypted_password, email_confirmed_at, raw_user_meta_data)
VALUES (
    gen_random_uuid(),
    'bolivar@diamelab.com',
    crypt('Bolivar2024!', gen_salt('bf')),
    NOW(),
    '{"full_name": "Vendedor Ciudad Bolivar", "role": "vendedor_bolivar", "sede": "Ciudad Bolivar"}'::jsonb
);

-- VENDEDOR CIUDAD GUAYANA
INSERT INTO auth.users (id, email, encrypted_password, email_confirmed_at, raw_user_meta_data)
VALUES (
    gen_random_uuid(),
    'guayana@diamelab.com',
    crypt('Guayana2024!', gen_salt('bf')),
    NOW(),
    '{"full_name": "Vendedor Ciudad Guayana", "role": "vendedor_guayana", "sede": "Ciudad Guayana"}'::jsonb
);
...
VALUES (
    gen_random_uuid(),
    'maturin@diamelab.com',
    crypt('Maturin2024!', gen_salt('bf')),
    NOW(),
    '{"full_name": "Vendedor Maturin", "role": "vendedor_maturin", "sede": "Maturin"}'::jsonb
);
*/
...
-- 1. Bucket: "comprobantes-pagos"
--    - Carpeta para guardar comprobantes de pago
--    - Policy: Authenticated users can upload
--    - Policy: Authenticated users can read their own files
--    - Public: true (para poder ver las URLs publicas)

-- 2. Bucket: "retenciones-iva"
--    - Carpeta para guardar retenciones de IVA
--    - Policy: Authenticated users can upload
--    - Policy: Authenticated users can read their own files
--    - Public: true

-- 3. Bucket: "retenciones-islr"
--    - Carpeta para guardar retenciones de ISLR
--    - Policy: Authenticated users can upload
--    - Policy: Authenticated users can read their own files
--    - Public: true

-- SQL para crear buckets (ejecutar en Supabase):
/*
INSERT INTO storage.buckets (id, name, public) VALUES
('comprobantes-pagos', 'comprobantes-pagos', true),
('retenciones-iva', 'retenciones-iva', true),
('retenciones-islr', 'retenciones-islr', true);

-- Policies para comprobantes-pagos
CREATE POLICY "Authenticated users can upload comprobantes" ON storage.objects
    FOR INSERT TO authenticated WITH CHECK (bucket_id = 'comprobantes-pagos');

CREATE POLICY "Authenticated users can read comprobantes" ON storage.objects
    FOR SELECT TO authenticated USING (bucket_id = 'comprobantes-pagos');

-- Policies para retenciones-iva
CREATE POLICY "Authenticated users can upload retenciones-iva" ON storage.objects
    FOR INSERT TO authenticated WITH CHECK (bucket_id = 'retenciones-iva');

CREATE POLICY "Authenticated users can read retenciones-iva" ON storage.objects
    FOR SELECT TO authenticated USING (bucket_id = 'retenciones-iva');

-- Policies para retenciones-islr
CREATE POLICY "Authenticated users can upload retenciones-islr" ON storage.objects
    FOR INSERT TO authenticated WITH CHECK (bucket_id = 'retenciones-islr');

CREATE POLICY "Authenticated users can read retenciones-islr" ON storage.objects
    FOR SELECT TO authenticated USING (bucket_id = 'retenciones-islr');
*/

-- ============================================================
-- CONFIGURACION ADICIONAL EN SUPABASE (Manual)
-- ============================================================
-- 1. Ve a Authentication > Settings > Email
--    - Desactiva "Confirm email" para pruebas (opcional)
--    - O deja activado y confirma emails manualmente
--
-- 2. Ve a Authentication > URL Configuration
--    - Site URL: https://tu-proyecto.vercel.app
--    - Redirect URLs: https://tu-proyecto.vercel.app/*.html
--
-- 3. Ve a API > Project URL y copia:
--    - SUPABASE_URL
--    - SUPABASE_ANON_KEY (anon public)
--
-- 4. Pega estos valores en js/supabase-config.js
